...
export default function NotesApp() {
    ...
    try {
      ...
      const formattedNotes = resNote.data.map((note) => ({
        ...
        img: note.image && note.image.startsWith('http')
          ? note.image
          : "image/logo.png",
        version: note.version, // ✅ Lấy version về
      }));
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // và trường hợp thêm ảnh mới (imageFile tồn tại)
      if (imageFile) {
        // Tải ảnh mới lên (Nếu có file mới)
        imageUrl = await uploadToCloudinary();
      } else if (!formData.img) {
        // Trường hợp người dùng xóa ảnh cũ và không upload ảnh mới
        imageUrl = '';
      }
      // Trường hợp người dùng giữ nguyên ảnh cũ: imageUrl = formData.img

      if (editingId) {
        // --- CHỈNH SỬA NOTE (UPDATE) ---
        const res = await notesAPI.update(editingId, {
          original: formData.original,
          translate: formData.translate,
          image: imageUrl,
          version: formData.version, // 🔑 Gửi version hiện tại lên server
        });
        
        // Cập nhật State thành công với dữ liệu và version mới từ Server
        setNotes(notes.map(note => note.id === editingId ? {
          ...note,
          original: res.data.note.original,
          translate: res.data.note.translate,
          img: res.data.note.image,
          version: res.data.note.version, // ✅ Lấy version mới nhất từ API
        } : note));
        setEditingId(null);
        ...
        // Thêm Note mới vào State với Version được server khởi tạo
        setNotes([...notes, {
          id: res.data._id,
          original: res.data.original,
          translate: res.data.translate,
          img: res.data.image,
          version: res.data.version, // ✅ Lấy version khởi tạo từ API
        }]);
      }

      // Dọn dẹp form sau khi thành công
      setFormData({ original: '', translate: '', img: '', version: 1 });
      setImageFile(null);
      setIsAdding(false);
      ...
      // 🔑 XỬ LÝ OPTIMISTIC LOCKING: Lỗi 409
      if (err.response && err.response.status === 409) {
        alert("Ghi chú đã được cập nhật bởi một phiên làm việc khác. Vui lòng TẢI LẠI dữ liệu để tiếp tục sửa!");
        // 🛑 Force Reload Notes để client có version mới nhất
        await fetchNotes(); 
        // Sau khi tải lại, client có thể thử sửa lại lần nữa
      } else {
  ...
  const handleDelete = async (id) => {
    try {
      ...
      // Cập nhật state (đã đúng)
      const newNotes = notes.filter(note => note.id !== id);
      setNotes(newNotes); 
      // Xử lý chuyển slide
      if (currentSlide >= newNotes.length) {
        setCurrentSlide(Math.max(0, newNotes.length - 1));
      }
    } catch (err) {
